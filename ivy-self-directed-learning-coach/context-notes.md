...
## 글분석 고급 A, B, C PDF 교재 추가

- 요청 대상은 `담쟁이 학습코칭 교재/자기주도 글분석력/글분석고급A.pdf`, `글분석고급B.pdf`, `글분석고급C.pdf`와 운영 `#analysis` 화면이다.
- 세 PDF 모두 106쪽이며 4쪽부터 104쪽까지 짝수 쪽에 본문 51개, 다음 홀수 쪽에 마인드맵·서술형 활동지가 배치되어 있다.
- PDF에 한글 텍스트 레이어가 있어 PyMuPDF로 직접 추출했다. 페이지 헤더(`글분석능력 (글분석 고급-A)`) 패턴은 정규식으로 제거했다.
- 고급 A는 `advanced-a`, 고급 B는 `advanced-b`, 고급 C는 `advanced-c` 키를 사용한다. ID 형식은 `advanced-a-01`~`advanced-a-51`.
- 고급 A PDF 34·70, 36·72, 32·76쪽은 동일 지문이 중복 수록되어 있다. 교재 원본 구성을 유지한다.
