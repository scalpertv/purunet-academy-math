...
- 요청 대상은 `/nakan-middle1-math-workbook-ai-math`의 최종 답안 입력 영역이다. 현재 43개 생성 유형이 `problemAnswerMode()`에서 모두 `choice`로 강제되고 `makeChoices()`가 항상 보기 4개를 만든다.
- 기존 CSS와 채점 코드에는 2열·3열 선택지와 주관식 입력 영역의 기반이 남아 있다. 새 화면 체계를 별도로 만들지 않고 이 구조를 복원·정리해 기존 교사형 풀이, 정답 가림, 전문 시각화와 충돌하지 않게 한다.
- 응답 형식은 문제의 수학적 성격으로 결정한다. 참·거짓이나 두 범주 판별은 2지, 제한된 개념·분류 선택은 3지, 계산형·오개념 구별은 4지, 식 작성·정확한 수치 산출은 주관식으로 배정한다.
- 모든 객관식 보기 값은 KaTeX 렌더러를 거치며, 주관식은 학생이 일반 키보드 표기 `-3/4`, `2x+1`, `5π`를 입력하면 입력 아래에서 KaTeX 미리보기로 확인할 수 있게 한다.
- 43개 문제 유형을 한 번씩 생성한 결과 2지선다 1개, 3지선다 2개, 4지선다 30개, 주관식 10개 유형으로 분류됐다. 정수 비교 유형은 생성된 문항 성격에 따라 3지선다, 4지선다, 주관식으로 동적으로 바뀐다.
- 객관식은 문제별 보기 수만 생성하고 중복 보기와 복수 정답을 막았다. 주관식은 Enter 제출, 빈 답안 방지, 수식 미리보기, 공통 채점·오답 코칭, 제출 뒤 입력 잠금을 연결했다.
- 수식 정규화로 `2*3^2*5`처럼 키보드로 입력한 답과 화면의 전문 수학 표기를 비교할 수 있다. 분수, 괄호가 있는 분자, 거듭제곱, 제곱·세제곱 단위도 KaTeX 변환 범위에 포함했다.
- 교사형 풀이 마지막 단계와 AI 코치 문구는 객관식에서는 보기 대조, 주관식에서는 답안 수식 정리와 입력 검산을 안내하도록 분리했다. 정답은 제출 전 DOM에 표시하지 않는다.
- `node --check js/nakan-middle1-math-workbook-ai-math.js`, `npm.cmd run build`, 43개 전체 유형 Playwright 순회가 통과했다. 모바일 390×844 대표 검사에서도 네 응답 형식, KaTeX 미리보기, 가로 넘침 0, 콘솔 오류 0을 확인했다.
- 아카데미 저장소 구현 커밋은 `7cb5c97 feat: add mixed KaTeX math responses`이다.
- 사용자 미커밋 변경을 제외한 `7cb5c97` 전용 임시 작업트리에서 Cloudflare Pages에 배포했다. 프리뷰 URL은 `https://d9a89d42.purunet-academy.pages.dev`이다.
- 운영 `https://purunet-academy.pages.dev/nakan-middle1-math-workbook-ai-math`에서 HTTP 200, 혼합 응답형 제목, 2·3·4개 보기, KaTeX 선택지, 주관식 입력·미리보기, 제출 전 정답 비노출, 모바일 가로 넘침 0, 콘솔 오류 0을 재확인했다.

## 초등 6학년 독서논술 6~20단계 확장 (2026-06-07)
- 요청 목표는 `/reading-essay-app`의 초등학교 6학년 추천도서 가운데 6~20단계를 기존 6단계 학습 루틴으로 실제 학습할 수 있게 만드는 것이다.
- 직접 대상은 아카데미 저장소의 `js/reading-essay-elem6-catalog.js`와 `docs/reading-essay-elem6-curriculum-plan.md`이다.
- 각 단계는 도서 원문이나 출판사 소개문을 복제하지 않고, 도서의 주제와 교육적 쟁점을 바탕으로 자체 작성한 지문, 어휘 4개, 이해 퀴즈 3개, 생각 구조 힌트, 논술 주제 2개를 제공한다.
- 6~10단계는 편견·권력·공동체·비판적 읽기·에너지 위기, 11~15단계는 학교 전환·수학적 탐구·세계사·공감·역사적 판단, 16~20단계는 가족·돌봄·학교폭력 예방·토론·마음 돌봄을 중심으로 난도를 높인다.
- 기존 작업트리에는 이번 요청과 무관한 수학 웹북·리디렉션·배포 스크립트 변경이 있으므로 해당 파일은 수정하거나 스테이징하지 않는다.
- 구현 결과 초6 추천도서 카탈로그 50권 가운데 1~20단계가 활성 콘텐츠이고 21~50단계는 추천도서명이 보이는 준비 중 카드로 유지된다.
- 6~20단계의 자체 지문 길이는 416~535자이며, 기존 1~5단계를 포함한 20권 전체의 최소 지문 길이는 396자이다. 각 도서는 어휘 4개, 4지선다 퀴즈 3개, 생각 구조 힌트 5개, 논술 주제 2개를 갖는다.
- `node --check js/reading-essay-elem6-catalog.js`, 50권·활성 20권·단계 중복·필수 필드 자동 검사, `npm.cmd run build`, `git diff --check`가 통과했다.
- 로컬 Playwright 검증에서 데스크톱 카드 50개, 활성 20개, 준비 중 30개, 20권 전체의 추천 출처·학습 자료 안내·6단계 메뉴·300자 이상 지문을 확인했다. 모바일 390×844의 가로 넘침은 0이고 콘솔 오류도 없었다.
- 아카데미 구현 커밋은 `2a764a3 feat: 초6 독서논술 20단계 확장`이다.
